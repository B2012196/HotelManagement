...
{
    public class PaymentCallBackModel(IFinanceService financeService, IBookingService bookingService, ILogger<PaymentCallBackModel> logger) : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            try
            {
                ...
                if (response.VnPaymentResponseModel.Success && response.VnPaymentResponseModel.VnPayResponseCode == "00")
                {
                ...
                else
                {
                    ...
                    //xóa invoice
                    var resultDeleteInvoice = await financeService.DeleteInvoice(response.VnPaymentResponseModel.InvoiceId);
                    ////xóa booking
                    var resultDeleteBooking = await bookingService.DeleteBooking(resultDeleteInvoice.BookingId);
                }
            }
